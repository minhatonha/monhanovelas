...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originWhitelist: ['https://playervipmaster.com', 'https://google.com'],            // If non-empty, requests not from an origin in this list will be blocked.
  ...
  if (req.method === 'OPTIONS') {
    // Pre-flight request. Verifique se a origem está na lista branca:
    if (corsAnywhere.originWhitelist.indexOf(origin) !== -1 ||
        corsAnywhere.originWhitelist.indexOf(referer) !== -1) {
      res.writeHead(200, cors_headers);
      res.end();
    } else {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('This origin is not whitelisted to access the proxy.');
    }
    return;
  }

  // Verifique se a origem ou referer estão na lista branca antes de prosseguir
  if (corsAnywhere.originWhitelist.indexOf(origin) === -1 &&
      corsAnywhere.originWhitelist.indexOf(referer) === -1) {
    res.writeHead(403, 'Forbidden', cors_headers);
    res.end('This origin is not whitelisted to access the proxy.');
  ...
  // Resto do código para processar a solicitação continua aqui...
};
