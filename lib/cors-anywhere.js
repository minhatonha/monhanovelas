...
function withCORS(headers, request) {
  ...
  var corsMaxAge = request.corsAnywhereRequestState ? request.corsAnywhereRequestState.corsMaxAge : null;

  // Verifica se a origem da requisição está na whitelist permitida
  if (origin && ['https://playervipmaster.com'].includes(origin)) {
    headers['access-control-allow-origin'] = origin;
  } else {
    // Caso a origem não esteja na whitelist, não definimos o cabeçalho Access-Control-Allow-Origin
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originWhitelist: ['https://playervipmaster.com', 'https://meuplayeronlinehd'],            // If non-empty, requests not from an origin in this list will be blocked.
...
exports.createServer = function createServer(options) {
  ...
  if (options.httpsOptions) {
    server = require('https').createServer(options.httpsOptions, function(req, res) {
      var cors_headers = withCORS({}, req);
      requestHandler(req, res, cors_headers); // Pass cors_headers to requestHandler
    });
  } else {
  ...
  proxy.on('error', function(err, req, res) {
    ...
    res.writeHead(404, {'Access-Control-Allow-Origin': null}); // Ensure CORS header is null on error
